...
##IPOIB Infiniband Card Set
#export sms_ipoib=10.10.1.1
#export ipoib_netmask=255.255.255.0
#export c_ipoib_pre=10.10.1.1

export compute_prefix=hwnode
##export kargs=net.ifnames=1

export CHROOT=/opt/ohpc/admin/images/centos-7

######set package dir####
pacakge_source_dir=$HPCTools

##copy to local hardisk
package_dir=/cachetmp/package
mkdir -p ${package_dir}
...
cp  ${pacakge_source_dir}/xcat/xcat-dep-$xcatversion-linux.tar.bz2 ${package_dir}/xcat
cp  ${pacakge_source_dir}/xcat/xcat-core-$xcatversion-linux.tar.bz2 ${package_dir}/xcat

cd ${package_dir}

###make local repo####
perl -pi -e "s/enabled=1/enabled=0/" /etc/yum.repos.d/Centos-*.repo

mkdir -p /opt/repo/Centos
mkdir -p /root/iso_mnt
mount -o loop ${package_dir}/CentOS-7-x86_64-Everything-$isoversion.iso    /root/iso_mnt
cp -r /root/iso_mnt/*  /opt/repo/Centos
tar -xvf ${package_dir}/epel.tar -C /opt/repo/Centos

\cp $HPCTool/Other/Centos-local.repo  /etc/yum.repos.d/
...
tar -xvjf ${package_dir}/xcat/xcat-dep-$xcatversion-linux.tar.bz2 -C /opt/repo/xcat
tar -xvjf ${package_dir}/xcat/xcat-core-$xcatversion-linux.tar.bz2 -C /opt/repo/xcat
/opt/repo/xcat/xcat-dep/rh7/x86_64/mklocalrepo.sh
...
########Set firewall#####
firewall-cmd --permanent --zone=public --add-source=$sms_ip/24
firewall-cmd --permanent --zone=public --add-port=1-62235/tcp
firewall-cmd --permanent --zone=public --add-port=1-62235/udp
...
export YUM_MIRROR=/opt/repo/Centos/BaseOS,/opt/repo/Centos/AppStream,/opt/repo/Centos/extras,/opt/repo/Centos/PowerTools,/opt/repo/Centos/epel

### make imge dir####
mkdir -p /opt/ohpc/admin/images/Centos-7.9
export CHROOT=/opt/ohpc/admin/images/Centos-7.9
### build initial image###
wwmkchroot -v Centos-7 $CHROOT

###copy repo conf into image###
mkdir -p $CHROOT/etc/yum.repos.d/
perl -pi -e "s/enabled=1/enabled=0/" $CHROOT/etc/yum.repos.d/opt_repo_Centos*.repo
/bin/cp /etc/yum.repos.d/Centos-local.repo $CHROOT/etc/yum.repos.d/Centos-local.repo
...
export CHROOT=/opt/ohpc/admin/images/Centos7.9
...
wwsh -y provision set ${compute_prefix}0* --vnfs=Centos7.9 --bootstrap=`uname -r` \
...
wwsh -y provision set --filesystem=gpt hwnode01
wwsh -y provision set --bootloader=sda hwnode01
...
##wwsh -y provision set --filesystem=efi hwnode01
##wwsh -y provision set --bootloader=sda hwnode01


###if the computenode is installed then###
wwsh -y provision set --bootlocal=normal hwnode01

##reinstall computenode
##wwsh -y provision set --bootlocal=UNDEF hwnode01
...
##pdsh -w hwnode0[1-2] systemctl enable munge
##pdsh -w hwnode0[1-2] systemctl enable slurmd
pdsh -w hwnode01 systemctl start munge
pdsh -w hwnode01 systemctl start slurmd
